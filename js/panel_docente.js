...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  initTabs();
  initForoDocente();
  initCampanaNotificaciones("docente");
  initEnviarNotifDocente();
});
...
window.abrirDetalle = async function(cursoId) {
  ...
  await cargarForoDocente(cursoId);
...
window.mostrarVista = function(nombre) {
  ...
  if (nombre === "labs") cargarLabsDocente();
  if (nombre === "calificaciones") cargarCalificacionesDocente();
  if (nombre === "calendario") cargarCalendarioDocente();
  if (nombre === "notificaciones") cargarNotificacionesDocente();
};
...
function initEnviarNotifDocente() {
  document.getElementById("btnEnviarNotifDoc")?.addEventListener("click", async () => {
    const titulo  = document.getElementById("notifTituloDoc")?.value.trim();
    const mensaje = document.getElementById("notifMensajeDoc")?.value.trim();
    const alerta  = document.getElementById("alertaNotifDoc");

    if (!titulo) { mostrarAlertaDoc(alerta, "Escribe un título."); return; }

    const { data: { user } } = await supabaseClient.auth.getUser();
    const { error } = await supabaseClient.from("notificaciones").insert({
      titulo, mensaje, rol_destino: "estudiante", creado_por: user.id
    });

    if (error) {
      mostrarAlertaDoc(alerta, "No se pudo enviar. Verifica que el administrador ejecutó el script SQL de notificaciones para docentes.");
      return;
    }

    document.getElementById("notifTituloDoc").value = "";
    document.getElementById("notifMensajeDoc").value = "";
    mostrarAlertaDoc(alerta, "¡Notificación enviada correctamente!", true);
    await cargarNotificacionesDocente();
  });
}

function mostrarAlertaDoc(el, msg, exito) {
  if (!el) return;
  el.textContent = msg;
...
async function cargarNotificacionesDocente() {
  const el = document.getElementById("listaNotifDoc");
  if (!el) return;
  ...
  if (error) {
    el.innerHTML = `<p class="sin-datos">Aún no se ejecutó el script SQL que habilita notificaciones para docentes.</p>`;
    return;
  }
  ...
  el.innerHTML = notifs.map(n => `
    <div class="fila-notif-admin">
      ...
      <button class="btn-peligro btn-mini" onclick="eliminarNotifDoc('${n.id}')">
        <i class="fas fa-trash"></i>
      </button>
    </div>
  `).join("");
...
window.eliminarNotifDoc = async function(id) {
...
// ══ LABORATORIOS ══════════════════════════════════════════════════════════════
async function cargarLabsDocente() {
  const el = document.getElementById("gridLabsDoc");
  if (!el) return;
...
// ══ FORO DEL CURSO ════════════════════════════════════════════════════════════
function initForoDocente() {
  document.getElementById("btnPublicarComentarioDoc")?.addEventListener("click", async () => {
    const texto = document.getElementById("foroTextoDoc")?.value.trim();
    if (!texto || !cursoActivo) return;
    const { data: { user } } = await supabaseClient.auth.getUser();
    const { error } = await supabaseClient.from("comentarios_curso").insert({
      curso_id: cursoActivo.id, autor_id: user.id, texto
    });
    if (error) { alert("No se pudo publicar el comentario."); return; }
    document.getElementById("foroTextoDoc").value = "";
    await cargarForoDocente(cursoActivo.id);
  });
}

async function cargarForoDocente(cursoId) {
  const el = document.getElementById("listaForoDoc");
  if (!el) return;
  el.innerHTML = `<p class="sin-datos"><i class="fas fa-spinner fa-spin"></i> Cargando comentarios...</p>`;

  const { data: comentarios, error } = await supabaseClient
    .from("comentarios_curso").select("*").eq("curso_id", cursoId)
    .order("creado_at", { ascending: false });

  if (error) {
    el.innerHTML = `<p class="sin-datos">El foro aún no está disponible. Ejecuta el script SQL de nuevas funciones en Supabase.</p>`;
    return;
  }
  if (!comentarios || comentarios.length === 0) {
    el.innerHTML = `<p class="sin-datos">Sin comentarios todavía. Inicia la conversación con tus estudiantes.</p>`;
    return;
  }

  const ids = [...new Set(comentarios.map(c => c.autor_id))];
  const { data: perfiles } = await supabaseClient
    .from("perfiles").select("user_id, nombre, apellido, rol").in("user_id", ids);
  const mapa = {};
  (perfiles || []).forEach(p => mapa[p.user_id] = p);

  el.innerHTML = comentarios.map(c => {
    const p = mapa[c.autor_id];
    const nombre = p ? `${p.nombre} ${p.apellido}` : "Usuario";
    return `
      <div class="foro-comentario">
        <div class="foro-comentario-top">
          <span class="foro-comentario-autor">${nombre} ${p?.rol === 'docente' ? '<i class="fas fa-chalkboard-teacher" title="Docente"></i>' : ''}</span>
          <span class="foro-comentario-fecha">${formatFecha(c.creado_at)}</span>
        </div>
        <p>${c.texto}</p>
      </div>`;
  }).join("");
...
// ══ CALIFICACIONES (consolidado por curso) ═══════════════════════════════════
async function cargarCalificacionesDocente() {
  const select = document.getElementById("selectCursoCalif");
  if (select && select.options.length !== cursosDocente.length) {
    select.innerHTML = cursosDocente.map(c => `<option value="${c.id}">${c.nombre}</option>`).join("");
    select.onchange = () => renderTablaCalificaciones(select.value);
  }
  if (cursosDocente.length > 0) await renderTablaCalificaciones(select?.value || cursosDocente[0].id);

  document.getElementById("btnExportarCalif").onclick = () => exportarCalificacionesCSV(select?.value);
}

async function renderTablaCalificaciones(cursoId) {
  const el = document.getElementById("tablaCalificaciones");
  if (!el || !cursoId) return;
  el.innerHTML = `<p class="sin-datos"><i class="fas fa-spinner fa-spin"></i> Calculando notas...</p>`;
  ...
  if (estudianteIds.length === 0) {
    el.innerHTML = `<p class="sin-datos">Este curso aún no tiene estudiantes inscritos.</p>`;
    return;
  }

  const { data: perfiles } = await supabaseClient
    .from("perfiles").select("user_id, nombre, apellido").in("user_id", estudianteIds);

  const { data: modulos } = await supabaseClient.from("modulos").select("id").eq("curso_id", cursoId);
  const moduloIds = (modulos || []).map(m => m.id);

  const { data: actividades } = moduloIds.length
    ? await supabaseClient.from("actividades").select("id").in("modulo_id", moduloIds)
    : { data: [] };
  const actividadIds = (actividades || []).map(a => a.id);

  const { data: tareas } = await supabaseClient.from("tareas").select("id").eq("curso_id", cursoId);
  const tareaIds = (tareas || []).map(t => t.id);

  const { data: entregas } = actividadIds.length
    ? await supabaseClient.from("entregas").select("estudiante_id, calificacion").in("actividad_id", actividadIds)
    : { data: [] };

  const { data: entregasTareas } = tareaIds.length
    ? await supabaseClient.from("entregas_tareas").select("estudiante_id, calificacion").in("tarea_id", tareaIds)
    : { data: [] };

  window._calificacionesActuales = (perfiles || []).map(p => {
    const notasEnt = (entregas || []).filter(e => e.estudiante_id === p.user_id && e.calificacion !== null);
    const notasTar = (entregasTareas || []).filter(e => e.estudiante_id === p.user_id && e.calificacion !== null);
    const todas = [...notasEnt, ...notasTar].map(n => Number(n.calificacion));
    const totalItems = actividadIds.length + tareaIds.length;
    const promedio = todas.length ? (todas.reduce((a, b) => a + b, 0) / todas.length) : null;
    return {
      nombre: `${p.nombre} ${p.apellido}`,
      calificados: todas.length,
      totalItems,
      promedio
    };
  });

  if (window._calificacionesActuales.length === 0) {
    el.innerHTML = `<p class="sin-datos">No hay estudiantes para mostrar.</p>`;
    return;
  }

  el.innerHTML = `
    <table class="tabla-notas">
      <thead><tr><th>Estudiante</th><th>Calificadas</th><th>Promedio</th></tr></thead>
      <tbody>
        ${window._calificacionesActuales.map(r => {
          const clase = r.promedio === null ? "" : r.promedio >= 14 ? "alto" : r.promedio >= 11 ? "medio" : "bajo";
          return `<tr>
            <td>${r.nombre}</td>
            <td>${r.calificados}/${r.totalItems}</td>
            <td>${r.promedio !== null ? `<span class="chip-promedio ${clase}">${r.promedio.toFixed(1)} pts</span>` : "—"}</td>
          </tr>`;
        }).join("")}
      </tbody>
    </table>`;
...
function exportarCalificacionesCSV(cursoId) {
  const filas = window._calificacionesActuales || [];
  if (filas.length === 0) { alert("No hay datos para exportar."); return; }
  const curso = cursosDocente.find(c => c.id === cursoId);
  let csv = "Estudiante,Calificadas,Total,Promedio\n";
  filas.forEach(r => {
    csv += `"${r.nombre}",${r.calificados},${r.totalItems},${r.promedio !== null ? r.promedio.toFixed(1) : ""}\n`;
  });
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
  const link = document.createElement("a");
  link.href = URL.createObjectURL(blob);
  link.download = `Calificaciones_${(curso?.nombre || "curso").replace(/\s+/g, "_")}.csv`;
  link.click();
}

// ══ CALENDARIO DE TAREAS Y ACTIVIDADES ════════════════════════════════════════
async function cargarCalendarioDocente() {
  const el = document.getElementById("listaCalendarioDoc");
  if (!el) return;
  el.innerHTML = `<p class="sin-datos"><i class="fas fa-spinner fa-spin"></i> Cargando calendario...</p>`;

  const cursoIds = cursosDocente.map(c => c.id);
  if (cursoIds.length === 0) {
    el.innerHTML = `<p class="sin-datos">Crea un curso primero para ver tu calendario.</p>`;
    return;
  }
  const cursosMap = {};
  cursosDocente.forEach(c => cursosMap[c.id] = c.nombre);

  const { data: tareas } = await supabaseClient
    .from("tareas").select("id, titulo, fecha_entrega, curso_id").in("curso_id", cursoIds);

  const { data: modulos } = await supabaseClient.from("modulos").select("id, curso_id").in("curso_id", cursoIds);
  const moduloCursoMap = {};
  (modulos || []).forEach(m => moduloCursoMap[m.id] = m.curso_id);
  const moduloIds = (modulos || []).map(m => m.id);

  const { data: actividades } = moduloIds.length
    ? await supabaseClient.from("actividades").select("id, titulo, fecha_entrega, modulo_id").in("modulo_id", moduloIds)
    : { data: [] };

  const eventos = [
    ...(tareas || []).filter(t => t.fecha_entrega).map(t => ({
      titulo: t.titulo, fecha: t.fecha_entrega, curso: cursosMap[t.curso_id] || "", tipo: "Tarea"
    })),
    ...(actividades || []).filter(a => a.fecha_entrega).map(a => ({
      titulo: a.titulo, fecha: a.fecha_entrega, curso: cursosMap[moduloCursoMap[a.modulo_id]] || "", tipo: "Actividad"
    }))
  ].sort((a, b) => new Date(a.fecha) - new Date(b.fecha));

  if (eventos.length === 0) {
    el.innerHTML = `<p class="sin-datos">No tienes tareas ni actividades con fecha límite todavía.</p>`;
    return;
  }

  el.innerHTML = eventos.map(ev => {
    const fecha = new Date(ev.fecha);
    const estado = urgencia(ev.fecha);
    const claseEstado = estado === "vencida" ? "vencido" : estado === "urgente" ? "hoy" : "";
    return `
      <div class="calendario-item ${claseEstado}">
        <div class="calendario-fecha-bloque">
          <div class="dia">${String(fecha.getDate()).padStart(2,"0")}</div>
          <div class="mes">${fecha.toLocaleDateString("es-PE",{month:"short"})}</div>
        </div>
        <div>
          <strong>${ev.titulo}</strong>
          <p style="margin:.2rem 0 0;font-size:.8rem;color:var(--texto2);">
            <i class="fas fa-${ev.tipo === 'Tarea' ? 'clipboard-list' : 'book-open'}"></i> ${ev.tipo} · ${ev.curso}
          </p>
        </div>
      </div>`;
  }).join("");
