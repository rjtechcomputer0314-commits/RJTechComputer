...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  initTabs();
  initForoEstudiante();
  initCampanaNotificaciones("estudiante");
});
...
window.abrirDetalleCurso = async function(cursoId) {
  ...
  await cargarForoEstudiante(cursoId);
};
...
window.mostrarVista = function(nombre) {
  ...
  document.querySelectorAll(".nav-item[data-vista]").forEach(b => {
    ...
    if (nombre === "notas") cargarMisNotas();
};
...
// ══ FORO DEL CURSO ════════════════════════════════════════════════════════════
function initForoEstudiante() {
  document.getElementById("btnPublicarComentarioEst")?.addEventListener("click", async () => {
    const texto = document.getElementById("foroTextoEst")?.value.trim();
    if (!texto || !cursoVistaActivo) return;
    const { data: { user } } = await supabaseClient.auth.getUser();
    const { error } = await supabaseClient.from("comentarios_curso").insert({
      curso_id: cursoVistaActivo.id, autor_id: user.id, texto
    });
    if (error) { alert("No se pudo publicar el comentario."); return; }
    document.getElementById("foroTextoEst").value = "";
    await cargarForoEstudiante(cursoVistaActivo.id);
  });
}

async function cargarForoEstudiante(cursoId) {
  const el = document.getElementById("listaForoEst");
  if (!el) return;
  ...
  if (error) {
    el.innerHTML = `<p class="sin-datos">El foro aún no está disponible. Pide al administrador que active la tabla "comentarios_curso".</p>`;
    return;
  }
  if (!comentarios || comentarios.length === 0) {
    el.innerHTML = `<p class="sin-datos">Sé el primero en comentar en este curso.</p>`;
    return;
  }
...
// ══ MIS NOTAS Y CERTIFICADOS ══════════════════════════════════════════════════
async function cargarMisNotas() {
  const el = document.getElementById("listaNotasEst");
  if (!el) return;
  el.innerHTML = `<p class="sin-datos"><i class="fas fa-spinner fa-spin"></i> Calculando tus notas...</p>`;

  if (cursosInscritos.length === 0) {
    el.innerHTML = `<p class="sin-datos">Aún no estás inscrito en ningún curso.</p>`;
    return;
  }
  ...
  const { data: modulos } = await supabaseClient
    .from("modulos").select("id, curso_id").in("curso_id", cursoIds);
  const moduloIds = (modulos || []).map(m => m.id);
  const moduloCursoMap = {};
  (modulos || []).forEach(m => moduloCursoMap[m.id] = m.curso_id);

  const { data: actividades } = moduloIds.length
    ? await supabaseClient.from("actividades").select("id, modulo_id, puntos").in("modulo_id", moduloIds)
    : { data: [] };
  const actividadCursoMap = {};
  (actividades || []).forEach(a => actividadCursoMap[a.id] = moduloCursoMap[a.modulo_id]);

  const { data: tareas } = await supabaseClient
    .from("tareas").select("id, curso_id, puntos").in("curso_id", cursoIds);

  const { data: entregas } = (actividades || []).length
    ? await supabaseClient.from("entregas").select("actividad_id, calificacion").eq("estudiante_id", user.id)
    : { data: [] };

  const { data: entregasTareas } = await supabaseClient
    .from("entregas_tareas").select("tarea_id, calificacion").eq("estudiante_id", user.id);

  // Agrupar todo por curso
  const porCurso = {};
  cursosInscritos.forEach(c => porCurso[c.id] = { totalItems: 0, calificados: 0, sumaNotas: 0, sumaPuntos: 0 });

  (actividades || []).forEach(a => {
    const cid = actividadCursoMap[a.id];
    if (!porCurso[cid]) return;
    porCurso[cid].totalItems++;
  });
  (tareas || []).forEach(t => {
    if (!porCurso[t.curso_id]) return;
    porCurso[t.curso_id].totalItems++;
  });

  (entregas || []).forEach(e => {
    if (e.calificacion === null || e.calificacion === undefined) return;
    const cid = actividadCursoMap[e.actividad_id];
    if (!porCurso[cid]) return;
    porCurso[cid].calificados++;
    porCurso[cid].sumaNotas += Number(e.calificacion);
  });
  (entregasTareas || []).forEach(e => {
    if (e.calificacion === null || e.calificacion === undefined) return;
    const tarea = (tareas || []).find(t => t.id === e.tarea_id);
    if (!tarea || !porCurso[tarea.curso_id]) return;
    porCurso[tarea.curso_id].calificados++;
    porCurso[tarea.curso_id].sumaNotas += Number(e.calificacion);
  });

  el.innerHTML = cursosInscritos.map(c => {
    const r = porCurso[c.id];
    const promedio = r.calificados > 0 ? (r.sumaNotas / r.calificados) : null;
    const progreso = r.totalItems > 0 ? Math.round((r.calificados / r.totalItems) * 100) : 0;
    const completo = r.totalItems > 0 && r.calificados >= r.totalItems;
    const clasePromedio = promedio === null ? "" : promedio >= 14 ? "alto" : promedio >= 11 ? "medio" : "bajo";

    return `
      <div class="tarjeta-nota-curso">
        <div class="tarjeta-nota-curso-top">
          <div>
            <strong>${c.nombre}</strong>
            <p style="font-size:.8rem;color:var(--texto2);margin:.2rem 0 0;">${c.docenteNombre ? "Docente: " + c.docenteNombre : ""}</p>
          </div>
          ${promedio !== null ? `<span class="chip-promedio ${clasePromedio}"><i class="fas fa-star"></i> Promedio: ${promedio.toFixed(1)} pts</span>` : `<span class="chip-promedio medio">Sin calificar aún</span>`}
        </div>
        <div class="barra-progreso-curso"><div style="width:${progreso}%"></div></div>
        <p style="font-size:.78rem;color:var(--texto2);margin:0 0 .6rem;">${r.calificados}/${r.totalItems || 0} actividades y tareas calificadas (${progreso}%)</p>
        ${completo
          ? `<button class="btn-certificado" onclick="generarCertificado('${c.id}')"><i class="fas fa-award"></i> Descargar certificado</button>`
          : `<span style="font-size:.78rem;color:#999;"><i class="fas fa-lock"></i> El certificado se habilita al completar el 100% del curso</span>`}
      </div>`;
  }).join("");
}

window.generarCertificado = function(cursoId) {
  const curso = cursosInscritos.find(c => c.id === cursoId);
  if (!curso) return;
  const nombreEstudiante = `${perfilActual?.nombre || ""} ${perfilActual?.apellido || ""}`.trim() || "Estudiante";
  ...
  grad.addColorStop(0, "#021A54");
  grad.addColorStop(1, "#0B2F87");
  ctx.fillStyle = grad;
  ...
  ctx.lineWidth = 2;
  ctx.strokeRect(70, 70, 1260, 850);

  ctx.textAlign = "center";
  ctx.fillStyle = "#d4af37";
  ctx.font = "bold 32px Arial";
  ctx.fillText("RJTechComputer", 700, 170);

  ctx.fillStyle = "#fff";
  ctx.font = "bold 54px Georgia";
  ctx.fillText("CERTIFICADO DE FINALIZACIÓN", 700, 280);

  ctx.font = "26px Arial";
  ctx.fillText("Se otorga el presente certificado a:", 700, 380);

  ctx.fillStyle = "#d4af37";
  ctx.font = "bold 50px Georgia";
  ctx.fillText(nombreEstudiante, 700, 460);

  ctx.fillStyle = "#fff";
  ctx.font = "26px Arial";
  ctx.fillText("por haber completado satisfactoriamente el curso:", 700, 540);

  ctx.font = "bold 36px Georgia";
  ctx.fillText(`"${curso.nombre}"`, 700, 600);

  ctx.font = "22px Arial";
  const fecha = new Date().toLocaleDateString("es-PE", { day: "2-digit", month: "long", year: "numeric" });
  ctx.fillText(`Otorgado el ${fecha}`, 700, 760);

  ctx.font = "italic 20px Arial";
  ctx.fillText(`Docente a cargo: ${curso.docenteNombre || "—"}`, 700, 810);

  const link = document.createElement("a");
  link.download = `Certificado_${curso.nombre.replace(/\s+/g, "_")}.png`;
